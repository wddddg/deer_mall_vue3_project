<template>
  <BasicTable @register="registerTable">
    ...
    <template #feedbackId="{ record }">
      <div class="link" @click="hasPermission('BUYER_FEEDBACK_GET')? goShopDetail(record) : '' ">{{ record.id }}</div>
    </template>
    ...
    <template #action="{ record }">
      <div
        ...
        v-if="hasPermission('BUYER_FEEDBACK_DELETE')"
        >删除</div
      >
    </template>
  </BasicTable>
...
<script lang="ts" setup>
  ...
  import { getBasicColumns, getFormConfig } from './customer.data';
  import { feedbackList, deleteFeedback } from './customer.api';
  ...
  const goShopDetail = (record) => {
    // if (!hasPermission('BUYER_FEEDBACK_GET')) {
    //   return;
    // }
    go('/feedback/manage/detail/' + record.id);
  };

<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" width="50%">
    <a-row :gutter="10">
...
<script lang="ts" setup>
  ...
  import { accountPremisionList } from '/@/views/merchant/account-list/accountList.api';
  ...
  const [registerDrawer, {}] = useDrawerInner(async (record) => {
    console.log(record);
    acoountInfo.value = record;
    getPermissionList(record.roles[0].id);
  });
  const getTree = () => {
  ...
  const getPermissionList = async (id) => {
    let res = await accountPremisionList(id);

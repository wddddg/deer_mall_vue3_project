<template>
  ...
  >
    ...
    <ScrollContainer>
      <ul :class="`${prefixCls}-module`">
        ...
        >
          ...
          /> -->
          <p :class="`${prefixCls}-module__name`" style="font-size: 14px">
            {{ t(item.name) }}
          </p>
        </li>
      </ul>
    ...
    <div :class="`${prefixCls}-menu-list`" ref="sideRef" :style="getMenuStyle">
      ...
      >
        <span class="text" style="margin-left: 40px;font-size:22px"> {{ title }}</span>
        <!-- <div class="anticon" style="margin-left: 15px">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      async function handleModuleClick(path: string, hover = false) {
        ...
        if (children[0].path === '/dashboard/analysis') return go(children[0].redirect);
        let paths = children[0].children?.filter((item) => !item.hideMenu);
        go(paths[0].path);
      }
      ...
      async function setActive(setChildren = false) {
        ...
        if (!path) return;
        if (path.split('/')[path.split('/').length - 1] === currentRoute.value?.params.path){
          let currentPath = path.split('/')
          currentPath[path.split('/').length - 1] = ':path(.*)'
          activePath.value = await getCurrentParentPath(currentPath.join('/'));
        }else{
